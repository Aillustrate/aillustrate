...
class TextGenerator(ABC):
    ...
    def set_constraints(self, suppress_words=None, begin_suppress_words=None, sequence_bias=None, forced_eos=None, bad_words=None):
        if suppress_words:
    ...
    def generate_text(self, text):
        ...
        output = self.tokenizer.decode(self.model.generate(input_ids, pad_token_id=self.tokenizer.eos_token_id, **self.generation_config)[0])
        cleanup()
        return output
...
class ConceptGenerator(TextGenerator):
    def __init__(self, concept_type,
        ...
        super().__init__(concept_type, model, tokenizer, topic, root, system_prompt, template, generation_config, constraints, batch_size, config_path)
