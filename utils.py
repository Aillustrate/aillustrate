...
def archive(path, result_path='result'):
    ...
    for fname in tqdm(image_names):
        shutil.move(os.path.join(path, fname), os.path.join(folder_to_archive_path, fname))
...
def update_config(topic, concept_type,
    ...
    trigger_words = lora_config.get('trigger_words', [])
    config_paths = [generation_config_concepts_path, generation_config_path, image_generation_config_path]
...
def parse_concept_config(topic, concept_type, config_path, default_path, config_name):
        ...
        if concept_type in topic_config:
            ...
            logging.warning(f'No {config_name} provided for {topic} {concept_type}. Using default {config_name} for {concept_type}.')
            concept_config = default_concept_config[concept_type]
        else:
            logging.warning(f'No {config_name} provided for {topic} {concept_type}. Using general default {config_name}.')
