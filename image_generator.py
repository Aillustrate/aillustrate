...
class ImageGenerator:
    ...
    def set_config(self, 
        ...
        self.use_prompt_embeddings = self.config.get('use_prompt_embeddings', use_prompt_embeddings)
        num_inference_steps = num_inference_steps or self.config.get('num_inference_steps', 25)
    ...
    def set_negative_prompt(self, negative_prompt):
        if negative_prompt:
            ...
            if not negative_prompt_path:
                logging.warning(f'No negative prompt provided. Using default.')
                negative_prompt_path = self.DEFAULT_NEGATIVE_PROMPT_PATH
            self.negative_prompt = parse_concept_config(self.topic, self.concept_type, negative_prompt_path, self.DEFAULT_NEGATIVE_PROMPT_PATH, 'negative prompt')
    ...
    def generate_by_prompt(self, 
        ...
        for count, seed in enumerate(seeds):
            ...
            generation_kwargs = dict(width=self.width, height=self.height, guidance_scale=guidance_scale, num_inference_steps=num_inference_steps, num_images_per_prompt=1, generator=torch.manual_seed(seed))
        ...
        if not prompt:
            ...
            for concept, prompts_for_concept in tq:
                ...
                for prompt in prompts_for_concept:
                    images.append(self.generate_by_prompt(prompt, save=save, display_images=False, series_name=concept, **kwargs))
        else:
    ...
    def generate_series(self, save:bool=True):
        ...
        for concept, prompts_for_concept in tq:
            tq.set_description(concept)
            for prompt in prompts_for_concept:
                images = self.generate(prompt, display_images=False, series_name=concept, save=save)
...
def get_pipe(
    ...
    lora_paths: List[str] = [], 
    config_path: str = 'config/image_generation_config.json') -> DiffusionPipeline:
    """
