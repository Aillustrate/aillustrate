...
def process_item(img1):
    ...
    for img2 in range(img1 + 1, len(image_files)):
        dist = 1 - np.dot(vec_from_files[img1], vec_from_files[img2]) / (np.linalg.norm(vec_from_files[img1]) * np.linalg.norm(vec_from_files[img2]))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Вывод названия всех одинаковых изображений")
    ...
    parser.add_argument("--th", type=float, default=0.1, help="threshold для сравнения векторов")
    ...
    image_files = [os.path.join(args.path, file) for file in os.listdir(args.path) if file.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))]
    ...
    print(f"Всего найдено {len(flat_list)/2} дубликатов ({round(len(flat_list)/len(image_files), 2)*100/2}%)")
    for file in range(0, len(flat_list), 2):
        print(str(flat_list[file]) + "\n"+ str(flat_list[file+1]) + "\n------------------")
    if args.delete:
